...
def run_cwltool_remote_wf(client):  # type: ignore[no-untyped-def]
    """Run a cwltool workflow with remote files, for other test functions to use."""
...
def test_run_cwltool_remote_wf(mocker, tmpdir):  # type: ignore[no-untyped-def]
...
attach_all_run_request = {
    ...
    "workflow_attachment": [
        (
            "workflow_attachment",
            ("ERR034597_1.small.fq.gz", RESOURCE_BASE_PATH.joinpath("ERR034597_1.small.fq.gz").open("rb")),
        ),
        (
            "workflow_attachment",
            ("ERR034597_2.small.fq.gz", RESOURCE_BASE_PATH.joinpath("ERR034597_2.small.fq.gz").open("rb")),
        ),
        (
            "workflow_attachment",
            ("trimming_and_qc.cwl", RESOURCE_BASE_PATH.joinpath("trimming_and_qc.cwl").open("rb")),
        ),
        ("workflow_attachment", ("trimmomatic_pe.cwl", RESOURCE_BASE_PATH.joinpath("trimmomatic_pe.cwl").open("rb"))),
        ("workflow_attachment", ("fastqc.cwl", RESOURCE_BASE_PATH.joinpath("fastqc.cwl").open("rb"))),
    ],
}


def test_run_cwltool_attach_all_files(mocker, tmpdir):  # type: ignore[no-untyped-def]
    client = anyhow_get_test_client(None, mocker, tmpdir)
...
def test_run_cwltool_fetch_remote_resource(mocker, tmpdir):  # type: ignore[no-untyped-def]
    client = anyhow_get_test_client(None, mocker, tmpdir)
    response = post_run(client, **fetch_remote_resource_run_request)  # type: ignore[arg-type]
    assert response.status_code == 200
    data = response.json()
    run_id = data["run_id"]
