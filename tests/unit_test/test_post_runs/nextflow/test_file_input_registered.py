...
def post_runs_file_input_registered() -> RunId:
    script_path = SCRIPT_DIR.joinpath("file_input_registered/post_runs.sh")
...
def test_file_input_registered(setup_test_server_registered_only_mode: None) \
        -> None:
    res_data = post_runs_file_input_registered()
    ...
    assert data["request"]["workflow_name"] == "nextflow_file_input"
    ...
    assert data["run_log"]["name"] == "nextflow_file_input"
